import numpy as np
import cv2
from multiresolution_decomposition import multiresolution_analysis
from registration import registration

dryrun = False

filenames = [["branchview", ".tif"], ["knoll", ".tif"]]
...
for arg in filenames:
    ...
    if not dryrun:
        ...
        for counter in range(len(nir_base)):
            np.save("output/{}_nir_base{}".format(filename, counter), nir_base[counter])
            np.save("output/{}_vis_base{}".format(filename, counter), rgb_base[counter])
        for counter in range(len(nir_detail)):
            np.save(
                "output/{}_nir_detail{}".format(filename, counter), nir_detail[counter]
            )
            np.save(
                "output/{}_vis_detail{}".format(filename, counter), rgb_detail[counter]
            )
